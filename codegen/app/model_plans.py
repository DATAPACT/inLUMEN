...
FASTER_WHISPER_PLAN: dict[str, Any] = {
    ...
    "inference_parameters": {
        ...
        # Preserve CPU capacity for Dagster and adjacent pipeline services.
        # Users can override this without changing the pinned model plan.
        "cpu_threads": 2,
        "num_workers": 1,
    },
    "quality_policy": {
    ...
    "resolution": {
        "source": "codegen-trusted-adapter-registry",
        "status": "resolved",
    },
}
...
def resolve_implementation_plan(
    ...
    *,
    ...
    """Return a verified adapter plan when a supported task is recognized."""
    candidate = dict(plan) if isinstance(plan, dict) else {}
    node_text = " ".join((str(label or ""), str(description or ""))).lower()
    # A node label is the strongest statement of its responsibility. Downstream
    # labels such as "Transcription Sentiment Report" mention upstream work
    # without being transcription nodes themselves.
    ...
    if is_asr_task_text(responsibility_text):
        return _merge_runtime_preferences(FASTER_WHISPER_PLAN, candidate)

    # Labels can be intentionally generic. In that case an explicit model plan
    # may still provide enough information to select a reviewed adapter.
    if _is_sentiment_task(task_text):
        return _merge_runtime_preferences(ROBERTA_SENTIMENT_PLAN, candidate)
    if is_asr_task_text(task_text):
        return _merge_runtime_preferences(FASTER_WHISPER_PLAN, candidate)
    return _proposed_custom_plan(candidate)


def trusted_adapter_id(plan: Any) -> str:
    candidate = plan if isinstance(plan, dict) else {}
    adapter_id = str(candidate.get("adapter_id") or "").strip()
    if candidate.get("schema_version") != IMPLEMENTATION_PLAN_SCHEMA:
        return ""
    if adapter_id not in {"faster-whisper", "transformers-roberta-sentiment"}:
        return ""
    return adapter_id


def model_plan_requires_exact_model(plan: Any) -> bool:
    candidate = plan if isinstance(plan, dict) else {}
    model_id = str(candidate.get("model_id") or "").strip()
    if not model_id:
        return False
    if trusted_adapter_id(candidate):
        return True
    resolution = candidate.get("resolution")
    return bool(
        candidate.get("execution_profile") == "trusted_heavy_model"
...
def _proposed_custom_plan(candidate: dict[str, Any]) -> dict[str, Any]:
    # Preserve unsupported/custom implementation payloads. They remain
    # advisory because only registry-resolved plans receive a trusted profile.
    return {key: _copy_value(value) for key, value in candidate.items()}


def is_asr_task_text(text: str) -> bool:
    """Recognize explicit ASR intent without matching every transcript consumer."""
    return any(
        token in text
        for token in (
            "speech-to-text",
...
def _without_mismatched_trusted_adapter(candidate: dict[str, Any]) -> dict[str, Any]:
    """Drop a stale registry adapter attached to a non-model responsibility."""
    if (
