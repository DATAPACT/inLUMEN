...
for (const scenario of [{preflight:true,status:200}, {preflight:false,status:200}, {preflight:false,status:524}]) {
  test(`browser workload preflight=${scenario.preflight}, HTTP=${scenario.status}`, {timeout:60000}, async () => {
    ...
    try {
      ...
      assert.equal(server.clears(),scenario.preflight?0:scenario.status===200?6:3);
      assert.ok(report.workspaces.every(w=>w.workspace_id===`personal-user${w.user_index}`));
      if (scenario.preflight || scenario.status===200) assert.ok([...server.workspaces.values()].every(w=>w.graph.nodes.length===2));
