...
The same browser verification also reproduced a second race: changing the node name while a replacement upload was in flight could revert the name when the upload completed. PropertiesPanel now merges asynchronous file patches into the latest selected-node data instead of the node captured at request start. A deferred-upload regression test demonstrates the old overwrite and verifies that newer name/description edits survive completion.

Verification/deployment results for `64a86b3` and `7b73d1d`:

- Backend unit suite: 327 tests, 317 passed and 10 optional integrations skipped. Separately, the real authenticated file workflow passed with revision preconditions enabled (local isolated Neo4j/MinIO data). The initial full-suite invocation with the developer `.env` inherited an invalid test Fernet key; rerunning the unit suite without unrelated environment settings passed.
- Frontend: 164 tests passed; typecheck and production build passed. Both new failure cases were demonstrated by failing regression tests before their corresponding fix.
- Deployed backend/frontend for the revision change, then frontend for the upload race. Codegen/runner and production authentication settings were unchanged.
- Real production browser test used the supported embedded SSO bootstrap with the user's current short-lived Keycloak session, and a temporary per-tab fetch wrapper selecting only the disposable workspace `163bb4a8-119c-4f15-aa25-087b61da5a5d`. Session and data responses came from the real authenticated backend; no authentication responses were mocked.
- Started with an empty canvas, dragged a Source from the library, changed its properties, uploaded WAV and CSV fixtures, edited CSV contents through Preview/Edit, replaced the CSV, saved a named version, reloaded, retrieved the replacement bytes, and saved another property change afterward. The source upload handler received generated File objects via its actual input/change event because Chrome's extension disallowed local-file selection; the native OS file-picker step was not verified.
- Browser trace showed strong numeric If-Match preconditions advancing across successful uploads, property saves, active-version snapshots, text edits, and named versions, while response ETags remained weak. The dedicated revision header survived the public proxy.
